...
<head>
    ...
    <title>Proposal from {{ $bid->guide->full_name }} - Travel Agency</title>
...
<body class="bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <!-- Back Button -->
        <div class="mb-6">
            <a href="{{ route('tourist-requests.show', $touristRequest) }}" class="text-blue-600 hover:text-blue-700 flex items-center">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <div class="p-4 bg-blue-50 rounded-lg">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-3xl font-bold text-blue-600">${{ number_format($bid->total_price, 2) }}</span>
                        </div>
                ...
                <div class="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach(is_array($bid->destinations_covered) ? $bid->destinations_covered : json_decode($bid->destinations_covered, true) as $destination)
                            <span class="px-3 py-1 bg-blue-100 text-blue-800 text-sm font-medium rounded-full">{{ $destination }}</span>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="lg:col-span-1">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 sticky top-6 space-y-4">
                    ...
                    <!-- Accept Button -->
                    <form action="{{ route('bids.accept', [$touristRequest, $bid]) }}" method="POST"
                        ...
                        @csrf
                        <button type="submit" class="w-full px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-semibold shadow-md hover:shadow-lg transition-all">
                            Accept Proposal
                        </button>
                    </form>
                    ...
                    <div x-data="{ showRejectForm: false }">
                        ...
                        <div x-show="showRejectForm" x-collapse class="mt-4 p-4 bg-red-50 rounded-lg">
                            <form action="{{ route('bids.reject', [$touristRequest, $bid]) }}" method="POST">
                                @csrf
