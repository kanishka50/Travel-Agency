...
<div class="bg-white rounded-xl shadow-sm border p-6">
    ...
    <div class="border-t border-b py-4 mb-4">
        ...
        <div class="mb-3">
            ...
            <div class="flex items-center">
                ...
                <input type="number" id="adults" value="2" min="1" max="{{ $plan->max_group_size }}"
                    ...
                    class="w-full h-10 text-center border-t border-b border-gray-300 focus:outline-none focus:ring-2 focus:ring-emerald-500">
        ...
        <div class="mt-4 p-3 bg-gray-50 rounded-lg">
            ...
            <div class="border-t pt-2 flex justify-between">
                ...
                <span id="total-price" class="text-xl font-bold text-emerald-600">${{ number_format($plan->price_per_adult * 2 * 1.1, 0) }}</span>
            </div>
        </div>
    </div>
    ...
    <!-- Booking Button -->
    <div class="mb-3">
        <p class="text-sm text-gray-600 mb-2 text-center">Select a date from the calendar below to book</p>
        @auth
            @if(auth()->user()->isTourist())
                <button type="button" id="book-now-price-btn" disabled
                    class="w-full py-3 bg-gray-300 text-gray-500 text-center font-semibold rounded-lg cursor-not-allowed transition">
                    Select Date to Book
                </button>
            @elseif(auth()->user()->isGuide())
                <div class="w-full py-3 bg-gray-100 text-gray-500 text-center font-semibold rounded-lg">
                    Guides cannot book tours
                </div>
            @endif
        @else
            <a href="{{ route('login') }}?redirect={{ urlencode(request()->url()) }}" class="block w-full py-3 bg-emerald-600 text-white text-center font-semibold rounded-lg hover:bg-emerald-700 transition">
                Login to Book
            </a>
        @endauth
    </div>
        ...
        @auth
            @if(auth()->user()->isTourist())
                <a href="{{ route('proposals.create', $plan->id) }}"
                   class="block w-full py-3 bg-teal-600 text-white text-center font-semibold rounded-lg hover:bg-teal-700 transition mb-3">
                ...
                @if($plan->min_proposal_price)
                    <p class="text-xs text-gray-500 text-center mb-3">
                        Minimum proposal price: ${{ number_format($plan->min_proposal_price, 0) }}
                    </p>
                @endif
            @endif
        @else
            <a href="{{ route('login') }}?redirect={{ urlencode(request()->url()) }}" class="block w-full py-3 bg-teal-600 text-white text-center font-semibold rounded-lg hover:bg-teal-700 transition mb-3">
    ...
    @else
        <a href="{{ route('login') }}?redirect={{ urlencode(request()->url()) }}" class="block w-full py-3 border-2 border-gray-300 text-gray-700 text-center font-semibold rounded-lg hover:border-red-500 hover:text-red-500 transition">
            Login to Save
        </a>
    @endauth
