...
<div class="bg-white rounded-xl shadow-sm border overflow-hidden hover:shadow-lg transition-shadow duration-300">
    <a href="{{ route('tour-packages.show', $plan->id) }}" class="block">
        <!-- Cover Image -->
        <div class="relative h-48 bg-gray-200 overflow-hidden">
            ...
            @if($plan->availability_type === 'always_available')
                <div class="absolute top-3 right-3">
                    <span class="px-2 py-1 bg-emerald-500 text-white text-xs font-semibold rounded-full">
                        Always Available
                    </span>
                </div>
            @else
        ...
        <div class="p-4">
            <!-- Title -->
            <h3 class="text-lg font-semibold text-gray-900 mb-2 line-clamp-2 hover:text-emerald-600 transition">
                {{ $plan->title }}
            </h3>
            ...
            <div class="flex flex-wrap gap-1 mb-3">
                @foreach(array_slice($plan->destinations ?? [], 0, 3) as $destination)
                    <span class="px-2 py-1 bg-emerald-50 text-emerald-700 text-xs rounded-full">
                        {{ $destination }}
                    </span>
                @endforeach
                @if(count($plan->destinations ?? []) > 3)
                    <span class="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full">
                        +{{ count($plan->destinations) - 3 }} more
                    </span>
                @endif
            </div>

            <!-- Guide Info -->
            <div class="flex flex-col text-sm text-gray-600 mb-3 pb-3 border-b">
                ...
                <span class="text-xs text-emerald-600 ml-5">{{ $plan->guide->guide_type_label ?? 'Guide' }}</span>
            </div>
            ...
            <div class="flex items-center justify-between">
                ...
                <span class="px-4 py-2 bg-emerald-600 text-white text-sm font-semibold rounded-lg hover:bg-emerald-700 transition">
                    View Details
                </span>
            </div>
