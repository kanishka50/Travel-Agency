...
<!-- Back Button -->
<div class="mb-6">
    <a href="{{ route('guide.complaints.index') }}" class="inline-flex items-center text-slate-600 hover:text-amber-600 transition-colors group">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <div class="flex justify-between items-start mb-4">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-1">Complaint #{{ $complaint->complaint_number }}</p>
                </div>
                <div class="text-right">
                    @if($complaint->status === 'open')
                        <span class="px-3 py-1 inline-flex text-sm font-semibold rounded-xl bg-amber-100 text-amber-700">
                            Open
                        </span>
                    @elseif($complaint->status === 'under_review')
                        <span class="px-3 py-1 inline-flex text-sm font-semibold rounded-xl bg-cyan-100 text-cyan-700">
                            Under Review
                        </span>
                    @elseif($complaint->status === 'resolved')
                        <span class="px-3 py-1 inline-flex text-sm font-semibold rounded-xl bg-emerald-100 text-emerald-700">
                            Resolved
                        </span>
                    @else
                        <span class="px-3 py-1 inline-flex text-sm font-semibold rounded-xl bg-slate-100 text-slate-700">
                            Closed
                        </span>
                    @endif
                </div>
            ...
            <div class="border-t border-slate-200 pt-4">
                <h3 class="font-semibold text-slate-900 mb-2">Complaint Description</h3>
            ...
            @if($complaint->evidence_files && count($complaint->evidence_files) > 0)
                <div class="border-t border-slate-200 pt-4 mt-4">
                    <h3 class="font-semibold text-slate-900 mb-3">Evidence Files</h3>
                    <div class="grid grid-cols-2 gap-3">
                        @foreach($complaint->evidence_files as $file)
                            <a href="{{ Storage::url($file) }}" target="_blank"
                               class="flex items-center p-3 border border-slate-200 rounded-xl hover:bg-slate-50 hover:border-amber-200 transition-all">
            ...
            @if($complaint->resolution_summary)
                <div class="border-t border-slate-200 pt-4 mt-4">
                    <h3 class="font-semibold text-slate-900 mb-2">Resolution Summary</h3>
                    <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                        <p class="text-slate-700">{{ $complaint->resolution_summary }}</p>
                        @if($complaint->resolved_at)
                            <p class="text-sm text-slate-500 mt-2">Resolved on {{ $complaint->resolved_at->format('M j, Y g:i A') }}</p>
                        @endif
                    </div>
                </div>
            @endif
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <h2 class="text-xl font-bold text-slate-900 mb-6">Communication Thread</h2>
                    ...
                    @foreach($complaint->responses as $response)
                        <div class="border rounded-xl p-4 {{ $response->responder_type === 'admin' ? 'bg-cyan-50 border-cyan-200' : 'bg-slate-50 border-slate-200' }}">
                            <div class="flex justify-between items-start mb-3">
                                ...
                                <div class="text-right">
                                    <span class="text-xs px-2 py-1 rounded-lg {{ $response->responder_type === 'admin' ? 'bg-cyan-100 text-cyan-700' : 'bg-slate-200 text-slate-700' }} font-medium">
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-sm text-slate-500">No responses yet. Our support team will respond soon.</p>
                </div>
            @endif
        </div>
        ...
        @if(!in_array($complaint->status, ['closed']))
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 class="text-lg font-bold text-slate-900 mb-4">Add Your Response</h3>
                <form action="{{ route('guide.complaints.addResponse', $complaint) }}" method="POST" enctype="multipart/form-data">
                    @csrf

                    <div class="mb-4">
                        <label for="response_text" class="block text-sm font-semibold text-slate-700 mb-2">Your Message</label>
                        <textarea name="response_text" id="response_text" required rows="4"
                                  placeholder="Type your response here..."
                                  class="w-full rounded-xl border-slate-200 focus:ring-amber-500 focus:border-amber-500"></textarea>
                        @error('response_text')
                    ...
                    <div class="mb-4">
                        <label for="attachments" class="block text-sm font-semibold text-slate-700 mb-2">Attachments (Optional)</label>
                        <input type="file" name="attachments[]" id="attachments" multiple
                               accept=".jpg,.jpeg,.png,.pdf,.doc,.docx"
                               class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 transition-colors">
                        <p class="mt-1 text-xs text-slate-500">JPG, PNG, PDF, DOC up to 10MB each</p>
                    </div>

                    <div class="flex justify-end">
                        <button type="submit"
                                class="px-6 py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-semibold shadow-lg shadow-amber-500/25 hover:shadow-amber-500/40 transition-all duration-300">
                            Send Response
                        </button>
                    </div>
                </form>
    ...
    <div class="space-y-6">
        <!-- Status Card -->
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <h3 class="font-bold text-slate-900 mb-4">Complaint Status</h3>
            ...
            <dl class="space-y-4">
                ...
                <div class="bg-slate-50 rounded-xl p-3">
                    ...
                    <dd class="mt-1">
                        @if($isComplainant)
                            <span class="px-2 py-1 text-xs font-semibold text-amber-700 bg-amber-100 rounded-lg">Complainant</span>
                        @else
                            <span class="px-2 py-1 text-xs font-semibold text-red-700 bg-red-100 rounded-lg">Defendant</span>
                        @endif
                    </dd>
                </div>
            </dl>
        ...
        @if($complaint->booking)
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 class="font-bold text-slate-900 mb-4">Related Booking</h3>
                <div class="space-y-3">
                    <div class="bg-slate-50 rounded-xl p-3">
                        <p class="text-sm text-slate-500">Booking Number</p>
                        <p class="font-semibold text-amber-600">{{ $complaint->booking->booking_number }}</p>
                    </div>

                    @if($complaint->booking->guidePlan)
                        <div class="bg-slate-50 rounded-xl p-3">
                            <p class="text-sm text-slate-500">Tour</p>
                            <p class="font-semibold text-slate-900">{{ $complaint->booking->guidePlan->title }}</p>
                        </div>
                    @endif

                    <a href="{{ route('guide.bookings.show', $complaint->booking) }}"
                       class="inline-flex items-center text-sm text-amber-600 hover:text-amber-700 font-semibold">
