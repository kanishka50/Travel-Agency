...
@section('page-title', 'File New Complaint')
...
<div class="mb-8">
    ...
    <p class="text-slate-500 mt-1">Please provide detailed information about your complaint</p>
</div>
...
<form action="{{ route('guide.complaints.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        ...
        <div>
            <label for="booking_id" class="block text-sm font-semibold text-slate-700 mb-2">Select Booking</label>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500">Provide detailed information about your complaint</p>
            </div>
        </div>
        ...
        <div id="tourist-selection-div" class="mb-4" style="display: {{ $selectedBooking ? 'none' : 'block' }};">
            <label for="tourist_selection" class="block text-sm font-semibold text-slate-700 mb-2">
                Select Tourist *
                <span class="text-slate-400 font-normal">(Required if no booking selected)</span>
            </label>
        ...
        <!-- Complaint Type -->
        <div class="mb-4">
            <label for="complaint_type" class="block text-sm font-semibold text-slate-700 mb-2">Complaint Type *</label>
        ...
        <div class="mb-4">
            <label for="description" class="block text-sm font-semibold text-slate-700 mb-2">
                ...
                <span class="text-slate-400 font-normal">(minimum 50 characters)</span>
            </label>
            <textarea name="description" id="description" required rows="8"
                      ...
                      class="w-full rounded-xl border-slate-200 focus:ring-amber-500 focus:border-amber-500">{{ old('description') }}</textarea>
            <p class="mt-1 text-sm text-slate-500">
                Character count: <span id="char-count" class="font-semibold">0</span> / 50 minimum
            </p>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <!-- Evidence Files -->
        <div>
            <label for="evidence_files" class="block text-sm font-semibold text-slate-700 mb-2">
                Supporting Evidence (Optional)
            </label>
            <div class="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center hover:border-amber-300 transition-colors">
                ...
                <input type="file" name="evidence_files[]" id="evidence_files" multiple
                       ...
                       class="hidden">
                <label for="evidence_files" class="mt-2 block text-sm font-semibold text-amber-600 cursor-pointer hover:text-amber-700">
...
fileInput.addEventListener('change', function() {
    ...
    if (this.files.length > 0) {
        Array.from(this.files).forEach(file => {
            ...
            fileItem.className = 'flex items-center justify-between bg-slate-50 p-3 rounded-xl';
            fileItem.innerHTML = `
