...
<div class="max-w-4xl mx-auto px-4 py-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('guide.plans.show', $plan) }}" class="ml-1 text-sm font-medium text-gray-500 hover:text-emerald-600 md:ml-2">{{ Str::limit($plan->title, 20) }}</a>
                </div>
            </li>
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Tour Plan</h1>
                <p class="text-gray-600 mt-2">Update your tour plan details</p>
            </div>
            <a href="{{ route('guide.plans.show', $plan) }}" class="text-gray-600 hover:text-gray-900">
    ...
    <form action="{{ route('guide.plans.update', $plan) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="flex items-center justify-end space-x-4 pt-6 border-t">
            <a href="{{ route('guide.plans.show', $plan) }}" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition">
                Cancel
            </a>
            <button type="submit" name="status" value="{{ $plan->status }}" class="px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition">
                Update Plan
            </button>
        </div>
    </form>
