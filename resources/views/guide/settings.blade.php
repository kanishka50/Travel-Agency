...
<div class="max-w-4xl">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
        ...
        <div class="flex items-start gap-6">
            <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center flex-shrink-0 shadow-lg shadow-amber-500/25">
            ...
            <div class="flex-1 grid sm:grid-cols-2 gap-4">
                <div class="bg-slate-50 rounded-xl p-4">
                    ...
                    <p class="text-slate-900 font-medium">{{ $guide->full_name }}</p>
                </div>
                <div class="bg-slate-50 rounded-xl p-4">
                ...
                <div class="bg-slate-50 rounded-xl p-4">
                    <label class="block text-sm font-medium text-slate-500 mb-1">Guide ID</label>
                    <p class="text-slate-900 font-mono font-medium">{{ $guide->guide_id_number }}</p>
                </div>
                <div class="bg-slate-50 rounded-xl p-4">
                    <label class="block text-sm font-medium text-slate-500 mb-1">Guide Type</label>
                    <p class="text-slate-900 font-medium">{{ $guide->guide_type_label }}</p>
                </div>
                <div class="bg-slate-50 rounded-xl p-4">
                    <label class="block text-sm font-medium text-slate-500 mb-1">Phone</label>
                    <p class="text-slate-900 font-medium">{{ $guide->phone ?? 'Not set' }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h2 class="text-lg font-bold text-slate-900">License & Insurance</h2>
        </div>

        <div class="grid sm:grid-cols-2 gap-6">
            <div class="bg-slate-50 rounded-xl p-4">
                <label class="block text-sm font-medium text-slate-500 mb-1">License Number</label>
                <p class="text-slate-900 font-medium">{{ $guide->license_number ?? 'Not provided' }}</p>
                @if($guide->license_expiry)
                    <p class="text-sm mt-1 {{ $guide->license_expiry->isPast() ? 'text-red-600' : 'text-slate-500' }}">
                        Expires: {{ $guide->license_expiry->format('M d, Y') }}
                    </p>
                @endif
            </div>
            <div class="bg-slate-50 rounded-xl p-4">
                <label class="block text-sm font-medium text-slate-500 mb-1">Insurance Policy</label>
                <p class="text-slate-900 font-medium">{{ $guide->insurance_policy_number ?? 'Not provided' }}</p>
                @if($guide->insurance_expiry)
                    <p class="text-sm mt-1 {{ $guide->insurance_expiry->isPast() ? 'text-red-600' : 'text-slate-500' }}">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
        ...
        <div class="grid sm:grid-cols-2 gap-6">
            ...
            <div class="bg-slate-50 rounded-xl p-4">
                <label class="block text-sm font-medium text-slate-500 mb-1">Account Status</label>
                <span class="inline-flex items-center px-2.5 py-1 {{ $user->status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }} text-sm font-semibold rounded-lg">
                    {{ ucfirst($user->status) }}
                </span>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        ...
        <div class="grid sm:grid-cols-2 gap-4">
            <a href="{{ route('guide.dashboard') }}" class="flex items-center p-4 border border-slate-200 rounded-xl hover:border-amber-200 hover:bg-amber-50/50 transition-all duration-200 group">
            ...
            <a href="{{ route('guide.plans.index') }}" class="flex items-center p-4 border border-slate-200 rounded-xl hover:border-amber-200 hover:bg-amber-50/50 transition-all duration-200 group">
                ...
                <div>
                    <p class="font-semibold text-slate-900 group-hover:text-amber-600 transition-colors">My Plans</p>
                    <p class="text-sm text-slate-500">Manage tour packages</p>
                </div>
            </a>
            <a href="{{ route('guide.bookings') }}" class="flex items-center p-4 border border-slate-200 rounded-xl hover:border-amber-200 hover:bg-amber-50/50 transition-all duration-200 group">
                ...
                <div>
                    <p class="font-semibold text-slate-900 group-hover:text-amber-600 transition-colors">Bookings</p>
                    <p class="text-sm text-slate-500">View your bookings</p>
                </div>
            </a>
            <a href="{{ route('guide.vehicles.index') }}" class="flex items-center p-4 border border-slate-200 rounded-xl hover:border-amber-200 hover:bg-amber-50/50 transition-all duration-200 group">
                ...
                <div>
                    <p class="font-semibold text-slate-900 group-hover:text-amber-600 transition-colors">My Vehicles</p>
                    <p class="text-sm text-slate-500">Manage vehicles</p>
                </div>
            </a>
        </div>
    </div>
