...
<div class="mb-8">
    ...
    <p class="text-slate-500 mt-1">View and manage your tour proposals</p>
</div>
    ...
    <div class="border-b border-slate-200">
        <nav class="flex space-x-8 px-6" aria-label="Tabs">
            ...
            <a href="{{ route('tourist.proposals.index', ['status' => 'pending']) }}"
               class="border-b-2 {{ request('status') === 'pending' ? 'border-amber-500 text-amber-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300' }} py-4 px-1 text-sm font-semibold">
                Pending
            </a>
...
@if($proposals->isEmpty())
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-12 text-center">
        ...
        <p class="text-slate-600 max-w-md mx-auto mb-6">You haven't sent any proposals yet. Browse tours and send a proposal to negotiate your perfect trip!</p>
                        ...
                        <div class="flex-1">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                <div class="bg-slate-50 rounded-xl p-3">
                                    <span class="text-slate-500 text-xs">Dates</span>
                                ...
                                <div class="bg-slate-50 rounded-xl p-3">
                                    ...
                                    <p class="font-semibold text-slate-900">${{ number_format($proposal->proposed_price, 2) }}</p>
                                </div>
                                <div class="bg-slate-50 rounded-xl p-3">
                                    <span class="text-slate-500 text-xs">Submitted</span>
                                    <p class="font-semibold text-slate-900">{{ $proposal->created_at->format('M d, Y') }}</p>
                                </div>
                            </div>
